...
void lcd_send(uint8_t data, uint8_t rs)
{
	...
	if (iomode == 4) {
		lcd_iowrite4(data >> 4); // 0100 0000 -> 0000 0100 
		lcd_iowrite4(data);
	} else {
		lcd_iowrite8(data);
	}
...
{
	uint8_t i;
	charnum &= 0x07;                                // 1 como ejemplo 
	lcd_command(E_SET_CGRAM_ADDR | (charnum << 3)); // 0000 0001 & 0000 0111 = 0000 0001 << 0000 1000 | 0100 0000 = 0100 1000    
	for (i = 0; i < 8; i++)
